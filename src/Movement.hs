...
tickEnemies seconds game = game{ player = killPlayer newPlayer
                               , enemies = aliveEnemies
                               , projectiles = movedProjectileList
                               , generator = snd genTuple}
  where
    ...
    newPlayer = if elem True
        (map (\x -> pointInBox (pLoc x) (x1,y1) (x2,y2)) projectileList)
            then (Player ppos pdir jump psize Dying1)
            else playerInstance
...
killEnemy (Enemy eType pos dir eSize dying@(Dead)) =
    (Enemy eType pos dir eSize Dead)
killEnemy (Enemy eType pos dir eSize dying) =
    (Enemy eType pos dir eSize (succ dying))
...
subFoldr2 entity player = rightCollisionEntityPlayer
    (leftCollisionEntityPlayer (upCollisionEntityPlayer
        (downCollisionEntityPlayer player entity) entity) entity) entity

subFoldr3 :: Entity a => a -> Enemy -> Enemy
subFoldr3 entity enemy = rightCollisionEntityEnemy
    (leftCollisionEntityEnemy (upCollisionEntityEnemy
        (downCollisionEntityEnemy enemy entity) entity) entity) entity
...
moveProjectile seconds (Projectile (x,y) (vx,vy) pSize) =
    (Projectile (x+vx*seconds, y+vy*seconds) (vx,vy) pSize)
